...
void ABoard::Tick(float DeltaTime)
{
    ...
    case PS_MOVING:
        ...
        break;
    case PS_GOT_BOTTOM: //llego abajo
        CoolLeft -= DeltaTime;
        if (CoolLeft <= 0.0f)
        {
...
void ABoard::NewPiece()
{
   ...
   /* Ingeniero = GetWorld()->SpawnActor<AIngenieroBloque>(AIngenieroBloque::StaticClass());
    BloqueFuego=GetWorld()->SpawnActor<AConstructorBloqueFuego>(AConstructorBloqueFuego::StaticClass());
    Ingeniero->SetConstructorBloque(BloqueFuego);
    Ingeniero->ConstruirBloque();

    APiece* PiezaGeneral = Ingeniero->GetBloqueGeneral();
    CurrentPiece = PiezaGeneral;
...
void ABoard::Builder()
{
    ...
    if (contador == 1)
    {
        for (int i = 1; i <= 2; i++)
        {
            BloqueFuego = GetWorld()->SpawnActor<AConstructorBloqueFuego>(FVector(0,Py,Pz), FRotator(0, 0, 0));
    ...
    if (contador == 2)
    {
        for (int i = 1; i <= 2; i++)
        {
            BloqueColorido = GetWorld()->SpawnActor<AConstructorBloqueColorido>(FVector(0, Py, Pz), FRotator(0, 0, 0));
            //Block_HIelo = GetWorld()->SpawnActor<ABlock_HIelo>(FVector(1000, 1000, 1000), FRotator(0, 0, 0));
            Ingeniero = GetWorld()->SpawnActor<AIngenieroBloque>(AIngenieroBloque::StaticClass());
            Ingeniero->SetConstructorBloque(BloqueColorido);
           // Ingeniero->ConstruirBloque();
            ABloqueGeneral* BloqueGeneral = Ingeniero->GetBloqueGeneral();
            BloqueGeneral->BloqueCaracteristicas();
            Py = Py * -1;
    ...
    if (contador == 3)
    {
        for (int i = 1; i <= 3; i++)
        {
            ...
            BloqueHielo = GetWorld()->SpawnActor<AConstructorBloqueHielo>(FVector(0, 0, Pz), FRotator(0, 0, 0));
            //Block_HIelo = GetWorld()->SpawnActor<ABlock_HIelo>(FVector(1000, 1000, 1000), FRotator(0, 0, 0));
            Ingeniero = GetWorld()->SpawnActor<AIngenieroBloque>(AIngenieroBloque::StaticClass());
            Ingeniero->SetConstructorBloque(BloqueHielo);
            // Ingeniero->ConstruirBloque();
            ABloqueGeneral* BloqueGeneral = Ingeniero->GetBloqueGeneral();
            BloqueGeneral->BloqueCaracteristicas();
        }
