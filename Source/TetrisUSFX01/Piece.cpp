...
APiece::APiece()
{
    ...
    struct FConstructorStatics
    {
        ...
        FConstructorStatics()
            ...
            , Color_7(TEXT("Material'/Game/Mesh/Material_6.Material_6'"))
...
void APiece::SpawnBlocks()
{
    ...
    const int GenerarAdapter = FMath::FRandRange(0, 3);
    ...
    for (auto&& YZ : YZs)
    {
        ...
        if (Index >=7) {
            if (i == 0) {
                i = 1;
                AdaptadorMovimientoAleatorio = NewObject<UAdaptadorMovimientoAleatorio_CA>(B);
                AdaptadorMovimientoAleatorio->SetLimitesMovimiento(0.0f, 10.0f, 0.0f);
        ...
        if (b)
        {
            for (auto&& Result : OutOverlaps)
            {
                ...
                if (Result.GetActor()->GetName() == "Cube_4") {
                    UE_LOG(LogTemp, Warning, TEXT("La puntuacion se a aumentado"))
                   //puntuacion = puntuacion + 10;
                  //  UE_LOG(LogTemp, Warning, TEXT("El Score de Puntuacion es: %d"),puntuacion)
                }
            }
            return true;
        }
